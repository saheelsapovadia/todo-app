...
const TodoPage = () => {
  ...
  const onDragOver = (event: DragOverEvent) => {
    ...
    if (over.data.current?.type === 'Todo') {
      ...
      const overTaskIndex = tasks.findIndex((task) => task.uid === overTask.uid);
      const activeTaskIndex = tasks.findIndex((task) => task.uid === activeTask?.uid);
  ...
  const handleAddTask = (newTask: TodoType) => {
    ...
    console.log(newTodo);
    axiosInstance.post('http://localhost:8080/api/todo', newTodo).then((response) => {
  ...
  const updateTodo = async (todo: TodoType):Promise<boolean> => {
    try{
      let res = await axiosInstance.put(`http://localhost:8080/api/todo/${todo.uid}`, todo);
  ...
  const getTodos = (userId: String) => {
    axiosInstance.get(`http://localhost:8080/api/getAllTodos/${userId}`).then((response) => {
  ...
  const deleteTask = (id: String) => {
    axiosInstance.delete(`http://localhost:8080/api/todo/${id}`).then((response) => {
    ...
    <>
      ...
      <DndContext onDragStart={onDragStart} onDragOver={onDragOver} onDragEnd={onDragEnd} sensors={sensors}>
        <div className="app-container flex ">
          {
            columns.map((column) => {
              return <Column column={column} todos={tasks.filter((task) => task.status === column.id)} expand={expand} />
            })
          }
        </div>
        <DragOverlay>
      ...
      {showDetail && <TodoDetail selectedTodo={activeTask} onClose={collapse} editMode={taskEditMode} addNewTodo={handleAddTask} updateTodo={handleUpdateTask} deleteTodo={deleteTask} />}
    </>
  )
}
